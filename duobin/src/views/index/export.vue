        ...
        <div class="order-content">
          ...
          <div class="order-list">
            <div class="order-item"
                 ...
                 :key="index"
                 @click="$router.push({path: '/export/orderDetail', query: {id: item.orderId}})">
              <div class="status">
                <div class="statusBtn"
...
<!--        </div>-->
      ...
      <div class="right">
        <div class="right-top">
          <div class="express" @click="$router.push('/export/placeOrder')">
            快速下单
          </div>
...
<script>
  ...
  export default {
    ...
    methods: {
       ...
       * */
      totalAmount() {
        this.$axios.get('/api/export/kanban/order/volume').then((res) => {
          // 本月业务量
          this.statistics[0].value = res.data.businessVolume
          // 未结汇金额
          this.statistics[1].value = res.data.nonExchangeAmount
          // 未开票总金额
          this.statistics[2].value = res.data.nonBillAmount
        }).catch((error) => {
          console.log(error)
        })
      },
       ...
       * */
      allOrder() {
        this.$router.push('/export/orderSearch')
      }
    }
  }
</script>
