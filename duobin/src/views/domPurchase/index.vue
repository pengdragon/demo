<template>
  <el-row>
    <div v-show="$route.path==='/fictitious/material/domPurchase'" class="content-wrapper">
      <search @searchEvent="searchEvent" :inputPlaceholder="'查询富森项目号/内部项目号'">
        <div slot="button" style="margin-bottom: 22px">
          <el-button type="primary" size="small" @click="$router.push('/fictitious/material/domPurchase/detail')" >新增</el-button>
  ...
  export default {
    ...
    methods: {
      getOrderList() {
        ...
        this.$axios.post('api/virtual/domesticProcure/itemList', jsonStr).then((res) => {
      ...
      dbClickCell(rows) {
        console.log(rows)
        this.$router.push({
          path: '/fictitious/material/domPurchase/detail',
