<template>
  <el-row>
    <div class="container-wrapper" v-show="$route.path==='/substitute/outDictateList2'">
      <b2bSearch @searchEvent='searchEvent' @searchSelect="searchSelect">
        <div slot="button" style="margin-bottom: 22px">
          <el-button size="small" type="primary"
                     @click="$router.push('/substitute/outDictateList2/newOutDictate?outwarehouseType='+'IMPORT')">
            新增进口交货
          </el-button>
          <el-button size="small" type="primary"
                     @click="$router.push('/substitute/outDictateList2/newOutDictate?outwarehouseType=' +'Local')">
            新增香港交货
          </el-button>
  ...
  export default {
    ...
    methods: {
      searchSelect(va) {
        ...
        this.$axios
          .post('/api/purchase/order/listDeliveryApply', data)
          .then(response => {
      ...
      dbGetCellMsg(row) {
        if (row.status === '0') {
          this.$router.push({ path: '/substitute/outDictateList2/newOutDictate?id=' + row.orderId })
        } else {
          this.$router.push({ path: '/substitute/outDictateList2/detail?id=' + row.orderId })
        }
      },
      ...
      bjFunc() {
        if (this.listSelect[0].status === '0') {
          this.$router.push({ path: '/substitute/outDictateList2/newOutDictate?id=' + this.listSelect[0].orderId })
        }
      },
      // 列表退回
      thFunc() {
        if (this.listSelect[0].status === 1) {
          ...
          this.$axios.post('/api/purchase/order/updateDeliveryApply', data)
            .then((response) => {
              this.getList()
            })
        } else {
      ...
      searchEvent(va) {
        ...
        this.$axios.post('/api/purchase/order/listDeliveryApply', data)
          .then((response) => {
            this.orderListData = response.data.list
            this.total = response.data.total
          })
      ...
      getList() {
        ...
        if (this.$route.path === '/substitute/outDictateList2') {
          this.$axios.post('/api/purchase/order/listDeliveryApply', data)
            .then((response) => {
              if (response.data) {
                this.orderListData = response.data.list
    ...
    created() {
      ...
      $route(v) {
        console.log()
        if (v.path === '/substitute/outDictateList2') {
          // 列表查询
          let data = {
            pageSize: 0,
            pageCount: 20
