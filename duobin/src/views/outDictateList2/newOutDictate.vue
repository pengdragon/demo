        ...
        <el-row class="content-wrapper">
          <el-col class="sendGoods form-wrapper">
            ...
            <el-form :label-width="'110px'"
              ...
              <el-form-item label="出货方式：" class="formItem Shipments">
                <el-radio v-model="formData.shipmentType" label="自提" @change="selfLifting(formData.shipmentType)">自提
                </el-radio>
                <el-radio v-model="formData.shipmentType" label="送货" @change="selfLifting(formData.shipmentType)">送货
                </el-radio>
                <el-radio v-model="formData.shipmentType" label="快递" @change="selfLifting(formData.shipmentType)">快递
                </el-radio>
              </el-form-item>
              <div class="self form-content" v-if="formData.shipmentType=='自提'">
                <el-form-item label="自提公司：" class="formItem">
                  <el-input v-model="formData.sendCompany"
                            class="inputBox"
              ...
              <div class="logistics form-content" v-if="formData.shipmentType=='送货'||formData.shipmentType=='快递'">
                ...
                <el-form-item label="地址：" class="formItem Shipments">
                  <el-select v-model="formData.provinceName" placeholder="省" class="inputName">
                    <el-option
                  ...
                  <el-select v-model="formData.cityName" placeholder="市" class="inputName">
                    <el-option
                      v-for="item in options"
                      :key="item.value"
                      :label="item.label"
                ...
                <el-form-item label="物流公司：" class="formItem" v-if="formData.shipmentType=='快递'">
                  <el-input v-model="formData.logisticsCompany"
                            class="inputBox"
                            suffix-icon="el-icon-search"
                            placeholder=""></el-input>
                </el-form-item>
                <el-form-item label="运费方式：" class="formItem" v-if="formData.shipmentType=='快递'">
                  <el-input v-model="formData.shipFeeType"
                            class="inputBox"
                            placeholder=""></el-input>
                </el-form-item>
          ...
          <el-col class="tableView-wrapper">
            ...
            <el-table
              ...
              <el-table-column
                prop="quantity" align="center"
                label="数量"
                width="120">
              </el-table-column>
              <el-table-column
                prop="ksendQty" align="center"
                label="可派货数量"
                width="120">
              </el-table-column>
              <el-table-column
    ...
    methods: {
      ...
      selectList() {
        if (this.formData.goods) {
          this.formData.goods = this.selectArray
        } else {
          this.formData.goods = this.selectArray
        }
      ...
      getOrder() {
        ...
        this.$axios.post('/api/purchase/order/selectDeliveryOrders', data).then((res) => {
      ...
      getOrderObj(orderId) {
        ...
        this.$axios.post('/api/purchase/order/queryDeliveryApply', data).then((res) => {
      ...
      submitHandle(v) {
        ...
        if (v === '保存') {
          ...
          this.$axios.post('/api/purchase/order/addDeliveryApply', this.formData)
            .then((response) => {
              this.$router.push({ path: '/substitute/outDictateList2' })
            })
        } else {
