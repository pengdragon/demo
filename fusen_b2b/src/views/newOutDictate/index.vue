<template>
  <div class="view-main">
    <layout :order="formData"
            :showStatus="true" @submitHandle='submitHandle' @uploadConfim='handleChange' :fileListProp="formData.file">
        ...
        <el-row class="content-wrapper">
          ...
          <el-col class="form-wrapper">
            ...
            <el-form :label-width="'100px'"
                     ...
                     size="small"
                     class="form-content">
              <el-form-item label="内部订单号：" class="formItem">
                <el-input v-model="formData.customOrder"
                          class="inputBox"
                          placeholder="">
                </el-input>
              </el-form-item>
              <!-- <el-form-item label="出库类型：" class="formItem">
                <el-radio v-model="formData.radio" label="1">进口订单</el-radio>
                <el-radio v-model="formData.radio" label="2">本地交货订单</el-radio>
              </el-form-item> -->
              <el-col :span="12">
                <el-form-item label="供应商：">
                  <el-select v-model="formData.supplier" filterable placeholder="请选择" style="width: 100%">
                    <el-option
              ...
              <el-form-item label="币别：" class="formItem" style="width: 150px;display: inline-block">
                <el-select v-model="formData.currency" placeholder="请选择" class="inputBox" style="width: 150px;">
                  <el-option
          ...
          <el-col class="tableView-wrapper">
            ...
            <el-button type="primary" size="small" @click="chooseOrder('选择订单')">选择订单</el-button>
              <el-button type="primary" size="small" plain @change="addDetails" style="position: relative">导入
               <input style="width: 100%;position: absolute;top:0;left: 0;right: 0;bottom: 0;opacity: 0;" type="file"
                      @change="addDetails"></input>
            </el-button>
            <el-button type="primary" size="small" plain @click="addList()">新增</el-button>

          </span>
            </div>
            <!-- <tabTmp :tabData="formData.lst"
              ...
              <el-table
                ...
                <el-table-column
                  ...
                  <template slot-scope="prop">
                    <el-input v-model="prop.row.orderNo" placeholder="入库订单号"></el-input>
                  </template>
                </el-table-column>

                <el-table-column
                  prop="model" align="center"
                  label="型号"
                  width="120">
                  <template slot-scope="prop">
                    <el-input v-model="prop.row.model" placeholder="型号"></el-input>
                  </template>
                </el-table-column>
                <el-table-column
                  prop="amount" align="center"
                  label="库存数量"
                  width="120">
                  <template slot-scope="prop">
                    <el-input v-model="prop.row.amount" placeholder="库存数量"></el-input>
                  </template>
                </el-table-column>
                <el-table-column
                ...
                <el-table-column
                  prop="packageNo" align="center"
                  label="箱号"
                  width="120">
                  <template slot-scope="prop">
                    <el-input v-model="prop.row.packageNo" placeholder="箱号"></el-input>
                  </template>
                </el-table-column>
                <el-table-column
                  prop="boxcount" align="center"
                  label="箱数"
                  width="120">
                  <template slot-scope="prop">
                    <el-input v-model="prop.row.boxcount" placeholder="箱数"></el-input>
                  </template>
                </el-table-column>
                <el-table-column
                  prop="pureWeight" align="center"
                  label="净重"
                  width="120">
                  <template slot-scope="prop">
                    <el-input v-model="prop.row.pureWeight" placeholder="净重"></el-input>
                  </template>
                </el-table-column>
                <el-table-column
                  prop="grossWeight" align="center"
                  label="毛重"
                  width="120">
                  <template slot-scope="prop">
                    <el-input v-model="prop.row.grossWeight" placeholder="毛重"></el-input>
                  </template>
                </el-table-column>
                <el-table-column
                ...
                >
                  <template slot-scope="scope">
                    <el-button @click="delDetail(scope.row,scope.$index)" type="text" size="small">删除</el-button>
          ...
          <el-col class="sendGoods form-wrapper" v-if="sendGoods">
            <div class="title">我要派货</div>
            <el-form :label-width="'110px'"
                     :model="sendGoodsData"
                     size="small"
                     class="form-content">
              <el-form-item label="出货方式：" class="formItem Shipments">
                <el-radio v-model="formData.radio" label="1" @change="selfLifting(formData.radio)">自提</el-radio>
                <el-radio v-model="formData.radio" label="2" @change="selfLifting(formData.radio)">送货</el-radio>
                <el-radio v-model="formData.radio" label="3" @change="selfLifting(formData.radio)">快递</el-radio>
              </el-form-item>
              <div class="self form-content" v-if="selfLiftingGoods">
                <el-form-item label="自提公司：" class="formItem">
                  <el-input v-model="sendGoodsData.carryCompany"
                            class="inputBox"
                ...
                <el-form-item label="自提人：" class="formItem">
                  <div style="width: 360px;">
                    <el-input v-model="sendGoodsData.carryName"
                              class="inputName"
                              placeholder="姓名"></el-input>
                    <el-input v-model="sendGoodsData.carryTel"
                              class="inputTel"
                              placeholder="联系电话"></el-input>
                  </div>
                </el-form-item>
                <el-form-item label="身份证号" class="formItem">
                  <el-input v-model="sendGoodsData.idCard"
                            class="inputBox"
                            placeholder=""></el-input>
                </el-form-item>
                <el-form-item label="备注：" class="formItem">
                  <el-input v-model="sendGoodsData.carryDesc"
                            class="inputBox"
                            placeholder=""></el-input>
                </el-form-item>
              </div>
              <div class="logistics form-content" v-if="logistics">
                <el-form-item label="送达公司：" class="formItem">
                  <el-input v-model="sendGoodsData.giveCompany"
                            class="inputBox"
                            suffix-icon="el-icon-search"
                            placeholder=""></el-input>
                </el-form-item>
                <el-form-item label="联系人：" class="formItem">
                  <div style="width: 360px;">
                    <el-input v-model="sendGoodsData.linkName"
                              class="inputName"
                              placeholder="姓名"></el-input>
                    <el-input v-model="sendGoodsData.linkTel"
                              class="inputTel"
                              placeholder="联系电话"></el-input>
                  </div>
                </el-form-item>
                <el-form-item label="地址：" class="formItem Shipments">
                  <el-select v-model="sendGoodsData.province" placeholder="省" class="inputName">
                    <el-option
                  ...
                  <el-select v-model="sendGoodsData.city" placeholder="市" class="inputName">
                    <el-option
                      v-for="item in options"
                      :key="item.value"
                      :label="item.label"
                  ...
                  <el-select v-model="sendGoodsData.area" placeholder="区" class="inputName">
                    <el-option
                      v-for="item in options"
                      :key="item.value"
                      :label="item.label"
                  ...
                  <el-input v-model="sendGoodsData.address"
                            class="inputBox"
                            placeholder="请输入详细地址"></el-input>
                  <el-checkbox v-model="sendGoodsData.checked" style="margin-left: 30px;">保险</el-checkbox>
                </el-form-item>
                <el-form-item label="快递月结卡号：" class="formItem">
                  <el-input v-model="sendGoodsData.cardNum"
                            class="inputBox"
                            placeholder=""></el-input>
                </el-form-item>
                <el-form-item label="预计送达日期：" class="formItem">
                  <div style="width: 360px;">
                    <el-date-picker
                      v-model="sendGoodsData.date"
                      type="date"
                      class="inputTel"
                      placeholder="选择日期">
                    </el-date-picker>
                    <el-time-select
                      v-model="sendGoodsData.time"
                      :picker-options="{
                      start: '08:30',
                      step: '00:15',
                      end: '18:30'
                ...
                <el-form-item label="物流公司：" class="formItem">
                  <el-input v-model="sendGoodsData.logisticsCompany"
                            class="inputBox"
                            suffix-icon="el-icon-search"
                            placeholder=""></el-input>
                </el-form-item>
                <el-form-item label="运费方式：" class="formItem">
                  <el-input v-model="sendGoodsData.freightType"
                            class="inputBox"
                            placeholder=""></el-input>
                </el-form-item>
                <el-form-item label="备注：" class="formItem">
                  <el-input v-model="sendGoodsData.desc"
                            class="inputBox"
                            placeholder=""></el-input>
                </el-form-item>
              </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      // 确认选择列表
      selectList() {
        this.formData.lst = this.formData.lst.concat(this.selectArray)
      ...
      delDetail(order, index) {
        if (order.id) {
          ...
          this.$axios.post('/api/purchase/order/deleteDetailRow', data).then((res) => {
            this.formData.lst.splice(index, 1)
          }).catch((error) => {
            console.log(error)
          })
        } else {
      ...
      // 新增
      addList() {
        this.formData.lst.push({})
      },
      // 选择订单列表

      getOrder() {
        let businessType
        if (this.$route.query.outwarehouseType === 'IMPORT') {
          businessType = 'FA'
        } else {
          businessType = 'FC'
        }
        let data = {
          'pageSize': 0,
          'pageCount': 10,
          'gatherSelcondition': '', // 多字段查询条件
          'businessType': businessType // FA进口/FC香港本地交货,必须传递
        }
        this.$axios.post('/api/purchase/order/selectOutWarehouseOrders', data).then((res) => {
      ...
      getOrderObj(orderId) {
        ...
        this.$axios.post('/api/purchase/order/queryOutWarehouseApply', data).then((res) => {
      ...
      submitHandle(v) {
        if (v === '保存') {
          this.formData.outwarehouseType = this.$route.query.outwarehouseType

          this.formData.operateType = 'ADD'
          console.log(this.formData)
          this.$axios.post('/api/purchase/order/addOutWarehouseApply', this.formData)
            .then((response) => {
              this.$router.push({ path: '/substitute/outDictateList' })
            })
        } else {
          this.formData.outwarehouseType = this.$route.query.outwarehouseType
