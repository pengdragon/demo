<template>
  <div class="content-wrapper">
    <div class="box">
      <!-- <el-row :gutter="10" v-if="showStatus">
        <div v-for="(item,i) in orderNode" :key="i">
          <el-col :span="3">
            <div class="item" :class="{'complete': currentNodeIndex >= i}">
              <div v-if="i!==0" class="triangle triangle-left"></div>
              <div v-if="i!==7" class="triangle triangle-right"></div>
              {{item}}
            </div>
          </el-col>
        </div> -->
  ...
  export default {
    props: {
      ...
      orderNode: {
        type: Array,
        default() {
          return ['下单', '受理中', '待确认', '已收货', '已付款', '全部报关', '已发货', '已签收']
        }
      },
      currentNodeIndex: {
        type: Number,
